...
class AppsFlyerS2SUploaderDoFn(MegalistaUploader):
    ...
    async def _send_http_request(self, session, payload, curr_retry):
        ...
        try:
            async with session.post(
                url, headers=headers, json=payload, raise_for_status=False, timeout=15
            ) as response:
                if response.status != 200:
                    if curr_retry < 3:
                        await asyncio.sleep(curr_retry)
                        return await self._send_http_request(
                            session, payload, curr_retry + 1
                        )
                    else:
        ...
        async with ClientSession(timeout=self.timeout) as session:
            ...
            for element in elements:
                task = asyncio.ensure_future(
                    self._prepare_and_send(session, element, success_elements)
                )
                tasks.append(task)
    ...
    def process(self, batch: Batch, **kwargs):
        ...
        future = asyncio.ensure_future(
            self._async_request_runner(batch.elements, success_elements), loop=loop
        )
        ...
        min_duration_sec = (
            len(batch.elements) / 500
        )  # Using Rate limitation = 500 per sec
