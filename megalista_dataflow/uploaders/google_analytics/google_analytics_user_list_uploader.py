...
class GoogleAnalyticsUserListUploaderDoFn(MegalistaUploader):
    ...
    def _create_list_if_doesnt_exist(
        self,
        analytics,
        web_property_id,
        view_ids,
        ...
        results = list(filter(lambda x: x["name"] == list_name, lists))
        if len(results) == 0:
    ...
    def _create_list(
        ...
        view_ids = [view_id]
        self._create_list_if_doesnt_exist(
            analytics,
            web_property_id,
            view_ids,
            user_id_list_name,
    ...
    @utils.safe_process(
        logger=logging.getLogger("megalista.GoogleAnalyticsUserListUploader")
    )
    ...
    def _do_upload_data(
        ...
        results = list(filter(lambda x: x["name"] == data_import_name, data_sources))
            ...
            body = "\n".join(
                [
                    ...
                    *[
                        ...
                        % (
                            row["user_id"],
                            row[custom_dim_field] if custom_dim_field else "buyer",
                        )
                        for row in rows
                    ],
                ]
