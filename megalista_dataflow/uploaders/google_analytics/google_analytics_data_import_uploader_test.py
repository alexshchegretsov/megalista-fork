...
def uploader(mocker):
    ...
    return GoogleAnalyticsDataImportUploaderDoFn(
        credentials, ErrorHandler(DestinationType.GA_DATA_IMPORT, MockErrorNotifier())
    )
...
def test_elements_uploading(mocker, uploader):
    ...
    uploader.process(
        Batch(
            execution,
            [
                {"user_id": "12", "cd1": "value1a", "cd2": "value2a"},
                {"user_id": "34", "cd1": "value1b", "cd2": "value2b"},
                {"user_id": "56", "cd1": None, "cd2": ""},
            ],
        )
    )
    ...
    print(media_bytes)
    assert (
        media_bytes == b"ga:user_id,ga:cd1,ga:cd2\n"
