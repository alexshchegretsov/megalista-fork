...
def test_exception_event_and_user_property(uploader):
    with requests_mock.Mocker() as m:
        ...
        destination = Destination(
            ...
            ["api_secret", "True", "True", "", "some_id", ""],
        )
        source = Source("orig1", SourceType.BIG_QUERY, ["", ""])
        execution = Execution(_account_config, source, destination)
        with pytest.raises(
            ValueError, match="GA4 MP should be called either for sending events"
        ):
...
def test_exception_app_and_web(uploader):
    with requests_mock.Mocker() as m:
        ...
        execution = Execution(_account_config, source, destination)
        with pytest.raises(
            ValueError, match="GA4 MP should be called either with a firebase_app_id"
        ):
            uploader.do_process(
                Batch(
                    execution,
...
def test_exception_no_id(uploader):
    with requests_mock.Mocker() as m:
        ...
        with pytest.raises(
            ...
            uploader.do_process(
                Batch(execution, [{"name": "event_name", "value": "123"}])
            )
...
def test_exception_app_event_without_app_instance_id(uploader):
    with requests_mock.Mocker() as m:
        ...
        with pytest.raises(
            ValueError,
            match="GA4 MP needs an app_instance_id parameter when used for an App Stream.",
        ):
            uploader.do_process(
                Batch(
                    execution,
...
def test_exception_web_event_without_client_id(uploader):
    with requests_mock.Mocker() as m:
        ...
        with pytest.raises(
            ValueError,
            match="GA4 MP needs a client_id parameter when used for a Web Stream.",
        ):
            uploader.do_process(
                Batch(
                    execution,
    ...
    with requests_mock.Mocker() as m:
        ...
        uploader.do_process(
            Batch(
                execution,
                [
                    {"user_ltv": "42", "client_id": "some_id"},
                    {"user_will_churn": "Maybe", "client_id": "some_id"},
                ],
            )
        )
        ...
        assert (
            m.last_request.json()["userProperties"]["user_will_churn"]["value"]
            == "Maybe"
        )
...
def test_succesful_web_user_property_call_with_user_id(uploader):
    with requests_mock.Mocker() as m:
        ...
        uploader.do_process(
            Batch(
                execution,
                [{"user_ltv": "42", "user_id": "Id42", "client_id": "someId"}],
            )
        )
...
def test_unsuccessful_null_api_secret(uploader, error_notifier):
    with requests_mock.Mocker() as m:
        ...
        execution = Execution(_account_config, source, destination)
        with pytest.raises(
            ValueError, match="GA4 MP should be called with a non-null api_secret"
        ):
            uploader.do_process(
                Batch(
                    execution,
