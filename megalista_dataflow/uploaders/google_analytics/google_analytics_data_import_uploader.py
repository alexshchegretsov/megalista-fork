...
class GoogleAnalyticsDataImportUploaderDoFn(MegalistaUploader):
    ...
    def _assert_all_list_names_are_present(any_execution):
        ...
        if len(destination) < 2:
            raise ValueError(
                "Missing destination information. Found " f"{len(destination)}"
            )
    ...
    def process(self, batch: Batch, **kwargs):
        ...
        self._do_upload_data(
            execution, web_property_id, data_import_name, ga_account_id, batch.elements
        )
    ...
    def prepare_csv(rows):
        ...
        body = "\n".join(
            [
                ",".join(
                    ["" if element is None else element for element in row.values()]
                )
                for row in rows
            ]
        )
    ...
    def _call_upload_api(
        ...
        media = MediaInMemoryUpload(
            bytes(csv, "UTF-8"), mimetype="application/octet-stream", resumable=True
        )
