...
class GoogleAnalyticsMeasurementProtocolUploaderDoFn(MegalistaUploader):
    ...
    def build_hit(self, batch: Batch, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            **{
                "cs": row[key]
                for key in row.keys()
                if key.startswith("campaign_source")
            },
            **{
                "cm": row[key]
                for key in row.keys()
                if key.startswith("campaign_medium")
            },
        }
        ...
        if hit_type == "event":
            ...
            for key in row.keys():
                if (
                    re.match("p([a]|[r]\d*[a-z]+)", key)  # product details
            ...
            logging.getLogger(
                "megalista.GoogleAnalyticsMeasurementProtocolUploader"
            ).error(error_message)
    ...
    @utils.safe_process(
        logger=logging.getLogger("megalista.GoogleAnalyticsMeasurementProtocolUploader")
    )
