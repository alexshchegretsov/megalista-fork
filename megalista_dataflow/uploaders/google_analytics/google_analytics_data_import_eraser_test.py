...
def eraser(error_notifier):
    ...
    return GoogleAnalyticsDataImportEraser(
        credentials, ErrorHandler(DestinationType.GA_DATA_IMPORT, error_notifier)
    )
...
def test_analytics_has_not_data_sources(mocker, eraser, caplog, error_notifier):
    ...
    service.management().customDataSources().list().execute.return_value = {"items": []}
    ...
    assert (
        "data_import_name - data import not found, please configure it in Google Analytics"
        in caplog.text
    )
