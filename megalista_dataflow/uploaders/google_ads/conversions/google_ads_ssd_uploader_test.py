...
def uploader(mocker):
    ...
    return GoogleAdsSSDUploaderDoFn(
        credentials,
...
def create_batch(account_config, currency_override, account_override):
    ...
    return Batch(
        execution,
        [
            {"hashed_email": "a@a.com", "time": _time1, "amount": "123"},
            {"hashed_email": "b@b.com", "time": _time2, "amount": "234"},
        ],
    )
...
def test_conversion_upload_account_and_currency_override(
    mocker, uploader, ssd_batch_with_account_and_currency_override
):
    mocker.patch.object(uploader, "_get_offline_user_data_job_service")
    mocker.patch.object(uploader, "_get_resource_name")
...
def test_conversion_mcc_account_override(
    mocker, uploader, ssd_batch_with_mcc_account_override
):
    mocker.patch.object(uploader, "_get_offline_user_data_job_service")
    mocker.patch.object(uploader, "_get_resource_name")
