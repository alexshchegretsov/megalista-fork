...
def uploader(mocker, error_notifier):
    ...
    return GoogleAdsOfflineUploaderDoFn(
        credentials,
...
def test_conversion_upload(mocker, uploader):
    ...
    element1 = {"time": time1, "amount": "123", "gclid": "456"}
    element2 = {"time": time2, "amount": "234", "gclid": "567"}
    ...
    uploader._get_oc_service.return_value.upload_click_conversions.return_value = (
        upload_return_mock
    )
    ...
    uploader._get_oc_service.return_value.upload_click_conversions.assert_called_once_with(
        request={
            "customer_id": "12345567890",
...
def test_upload_with_ads_account_override(mocker, uploader):
    ...
    batch = Batch(
        execution,
        [
            {"time": time1, "amount": "123", "gclid": "456"},
            {"time": time2, "amount": "234", "gclid": "567"},
        ],
    )
...
def test_error_notification(mocker, uploader, error_notifier):
    ...
    assert error_notifier.errors_sent == {
        execution: f"Error on uploading offline conversions: {error_message}."
    }
