...
class GoogleAdsECLeadsUploaderDoFn(MegalistaUploader):
    ...
    @utils.safe_process(
        logger=logging.getLogger("megalista.GoogleAdsECLeadsUploader")
    )  # Changed
    def process(self, batch: Batch, **kwargs):
        ...
        self._assert_ec_for_leads_enabled(ads_service, customer_id)
        ...
        response = self._do_upload(
            oc_service, execution, resource_name, customer_id, batch.elements
        )
        successful_users = [user for user in response.results if user.ListFields()]
    ...
    def _do_upload(
        self, oc_service, execution, conversion_resource_name, customer_id, rows
    ):
        logging.getLogger(_DEFAULT_LOGGER).info(
        ...
        conversions = [
            {
                ...
                "user_identifiers": [
                    ...
                    if k in ("hashed_email", "hashed_phone_number")
                ],
            }
        ...
        for batch in response_query:
            for row in batch.results:
                if (
                    ...
                    == True
                    and row.customer.conversion_tracking_setting.enhanced_conversions_for_leads_enabled
                    == True
                ):
                    return
                else:
