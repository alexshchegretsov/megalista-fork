...
def uploader(mocker, error_notifier):
    ...
    return GoogleAdsCustomerMatchContactInfoUploaderDoFn(
        credentials,
...
def test_upload_add_users(mocker, uploader, error_notifier):
    ...
    uploader._get_offline_user_data_job_service.return_value.add_offline_user_data_job_operations.return_value = (
        MagicMock().partial_failure_error
    ) = None
...
def test_upload_add_users_with_ads_account_override(mocker, uploader):
    ...
    destination = Destination(
        "dest1",
        DestinationType.ADS_CUSTOMER_MATCH_CONTACT_INFO_UPLOAD,
        ["user_list", "ADD", "FALSE", "", "override_account_id", ""],
    )
    source = Source("orig1", SourceType.BIG_QUERY, ["dt1", "buyers"])
    execution = Execution(_account_config, source, destination)
