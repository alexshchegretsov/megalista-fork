...
from mappers.batches_grouped_by_source_mapper import (
    BatchesGroupedBySourceCombineFn,
    BatchesGroupedBySourceMapper,
)
...
class TransactionalEventsResultsWriter(beam.PTransform):
    ...
    class _UploadData(MegalistaUploader):
        ...
        @utils.safe_process(
            logger=logging.getLogger("megalista.TransactionalEventsResultsWriter")
        )
        ...
        def _do_process(self, batch: Batch):
            executions = ExecutionsGroupedBySource(
                batch.execution.source.source_name, [batch.execution]
            )
    ...
    def expand(self, executions):
        return (
            ...
            >> beam.Map(lambda batch: (batch.execution.source.source_name, batch))
            | "Group by source name"
            >> beam.CombinePerKey(BatchesGroupedBySourceCombineFn())
            | "Encapsulate into object"
            >> beam.Map(BatchesGroupedBySourceMapper().encapsulate)
            | beam.ParDo(self._ElementsProcessor(self._error_handler))
            | beam.ParDo(
                self._UploadData(
                    self._dataflow_options,
                    self._transactional_type,
                    self._error_handler,
                )
            )
        )
