...
def uploader(mocker, error_notifier):
    ...
    return DisplayVideoCustomerMatchContactInfoUploaderDoFn(
        credentials,
...
def test_upload_add_users(mocker, uploader, error_notifier):
    ...
    uploader._get_dv_audience_service.return_value.list.return_value.execute.return_value = (
        None
    )
    uploader._get_dv_audience_service.return_value.create.return_value.execute.return_value = (
        audience
    )
    ...
    uploader._get_dv_audience_service.return_value.list.assert_called_once_with(
        advertiserId="advertiser_id", pageSize=1, filter='displayName : "list_name"'
    )
    ...
    uploader._get_dv_audience_service.return_value.create.assert_called_once_with(
        advertiserId="advertiser_id", body=test_create_resquest
    )
...
def test_upload_update_users(mocker, uploader, error_notifier):
    ...
    audience_list = {
        "firstAndThirdPartyAudiences": [
            {"firstAndThirdPartyAudienceId": 12345, "displayName": "list_name"}
        ]
    }
    ...
    uploader._get_dv_audience_service.return_value.editCustomerMatchMembers.return_value = (
        MagicMock()
    )
    ...
    uploader._get_dv_audience_service.return_value.editCustomerMatchMembers.assert_any_call(
        firstAndThirdPartyAudienceId=audience_list["firstAndThirdPartyAudiences"][0][
