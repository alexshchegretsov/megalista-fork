...
def uploader(error_notifier):
    ...
    return CampaignManagerConversionUploaderDoFn(
        credentials, ErrorHandler(DestinationType.CM_OFFLINE_CONVERSION, error_notifier)
    )
...
def test_conversion_upload(mocker, uploader):
    ...
    uploader._do_process(
        Batch(
            execution,
            [
                {"gclid": "123", "timestamp": "2021-11-30T12:00:00.000"},
                {"gclid": "456"},
            ],
        ),
        current_time,
    ...
    timestamp_micros = math.floor(
        datetime.strptime("2021-11-30T12:00:00.000", "%Y-%m-%dT%H:%M:%S.%f").timestamp()
        * 10e5
    )
...
def test_error_on_api_call(mocker, uploader, caplog, error_notifier):
    ...
    service.conversions().batchinsert().execute.return_value = {
        "hasFailures": True,
        "status": [{"errors": [{"code": "123", "message": "error_returned"}]}],
    }
