...
_dtypes: Dict[str, Dict[str, Any]] = {
    "CM_OFFLINE_CONVERSION": {
        ...
        "groups": [["gclid", "mobileDeviceId", "encryptedUserId", "matchId", "dclid"]],
    },
    ...
    "ADS_SSD_UPLOAD": {
        ...
        "groups": [["email", "phone", "mailing_address_first_name"]],
    },
    "ADS_ENHANCED_CONVERSION": {
        "columns": [{"name": ".*", "required": False, "data_type": "string"}],
        "groups": [],
    },
    ...
    "ADS_CUSTOMER_MATCH_USER_ID_UPLOAD": {
        "columns": [{"name": "user_id", "required": True, "data_type": "string"}],
        "groups": [],
    },
    "GA_USER_LIST_UPLOAD": {
        "columns": [{"name": ".*", "required": False, "data_type": "string"}],
    ...
    "GA_4_MEASUREMENT_PROTOCOL": {
        ...
        "groups": [["app_instance_id", "client_id"]],
    },
...
def _validate_required_columns(
    data_cols: List[str], destination_type: DestinationType
) -> List[str]:
    data_type = _dtypes[destination_type.name]
    data_type_cols = data_type["columns"]
    data_type_colnames = [
        col["name"] for col in data_type_cols if col["required"] == True
    ]
...
def update_data_types_not_string(
    df: pd.DataFrame, destination_type: DestinationType
) -> pd.DataFrame:
    ...
    for key in cols_to_change:
        if key in df.columns:
            dtypes_to_change[key] = list(
                filter(lambda col: col["name"] == key, data_type["columns"])
            )[0]["data_type"]
...
def process_by_destination_type(
    df: pd.DataFrame, destination_type: DestinationType
) -> pd.DataFrame:
...
def _join_custom_variables(df) -> pd.DataFrame:
    df["customVariables"] = (
        '{ "type": "'
        + df["customVariables.type"]
        + '", "value": "'
        + df["customVariables.value"]
        + '"}'
    )
