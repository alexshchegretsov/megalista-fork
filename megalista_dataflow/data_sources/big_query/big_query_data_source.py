...
class BigQueryDataSource(BaseDataSource):
    def __init__(
        ...
        if transactional_type is not TransactionalType.NOT_TRANSACTIONAL:
            if not bq_ops_dataset or bq_ops_dataset == "":
                raise Exception(
                    f'Missing bq_ops_dataset for this uploader. Source="{self._source_name}". Destination="{self._destination_name}"'
                )
    ...
    def _retrieve_data_non_transactional(
        ...
        if DataSchemas.validate_data_columns(cols, self._destination_type):
            ...
            logging.getLogger(_LOGGER_NAME).info(
                f"Data source ({self._source_name}): using {len(elements)} rows"
            )
    ...
    def _retrieve_data_transactional(
        ...
        table_name = self._get_table_name(executions.source.source_metadata, False)
        uploaded_table_name = self._get_table_name(
            executions.source.source_metadata, True
        )
        client = self._get_bq_client()
        ...
        cols = self._get_table_columns(client, table_name)
        logging.getLogger(_LOGGER_NAME).info(
            f"Destination Type: {self._destination_type}"
        )
        if DataSchemas.validate_data_columns(cols, self._destination_type):
            cols = DataSchemas.get_cols_names(cols, self._destination_type)
            ...
            if self._transactional_type == TransactionalType.UUID:
                ...
                raise Exception(
                    f'Unrecognized TransactionalType: {self._transactional_type}. Source="{self._source_name}". Destination="{self._destination_name}"'
                )
    ...
    def write_transactional_info(self, rows, execution: Execution):
        if len(rows) == 0:
            logging.getLogger(_LOGGER_NAME).info("No rows to insert. Skipping...")
        else:
            ...
            for i in range(0, len(all_bq_rows), _BIGQUERY_PAGE_SIZE):
                ...
                partial_results.append(
                    client.insert_rows(table, bq_rows, self._get_schema_fields())
                )
