...
class FileProvider:
    ...
    def _define_file_provider(self):
        ...
        if self._path.startswith("s3://"):
            ...
            # Local File
            return self._LocalFileProvider(
                self._path,
                self._dataflow_options,
                self._can_skip_read,
                self._source_name,
        ...
        raise NotImplementedError(
            f'Could not define File Provider. Path="{self._path}". Source="{self._source_name}"'
        )
        ...
        def read(self):
            if exists(self._cleaned_path):
                ...
                raise FileNotFoundError(
                    f'Could not find file. Path="{self._path}". Source="{self._source_name}"'
                )
    ...
    class _S3FileProvider(_BaseFileProvider):
        def __init__(
            ...
            logging.getLogger(_LOGGER_NAME).info(
                f'S3 File Provider initiated. Bucket: "{bucket_name}". Key="{key}"'
            )
        ...
        def write(self, data):
            response = self._s3_client.put_object(
                Bucket=self._bucket_name, Key=self._key, Body=data
            )
    ...
    class _GCSFileProvider(_BaseFileProvider):
        def __init__(
            ...
            logging.getLogger(_LOGGER_NAME).info(
                f'GCP Storage File Provider initiated. Bucket: "{bucket_name}". Path="{file_path}"'
            )
