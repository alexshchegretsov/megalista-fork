...
class FileDataSource(BaseDataSource):
    ...
    def _retrieve_data_non_transactional(
        self, executions: ExecutionsGroupedBySource
    ) -> List[DataRowsGroupedBySource]:
        source = executions.source
        # Get Data Source
        data_source = self._get_data_source(
            source.source_name, source.source_metadata[0]
        )
        # Get Data Frame
        df = data_source.get_data_frame(source.source_name, source.source_metadata[1])
        logging.getLogger(_LOGGER_NAME).info(
            f"Data source ({self._source_name}): using {len(df.index)} rows"
        )
        if df is not None:
            ...
            raise Exception(
                f'Unable to read from data source. Source="{source.source_name}".'
            )

    def _retrieve_data_transactional(
        self, executions: ExecutionsGroupedBySource
    ) -> List[DataRowsGroupedBySource]:
        source = executions.source
        # Get Data Source
        ...
        df_uploaded = data_source.get_data_frame(
            source.source_name, source.source_metadata[1], is_uploaded=True
        )
        ...
        if df is not None:
            ...
            logging.getLogger(_LOGGER_NAME).info(
                f"Data source ({self._source_name}): using {len(df_distinct.index)} rows"
            )
            elements = []
            ...
            raise Exception(
                f'Unable to read from data source. Source="{self._source_name}". Destination="{self._destination_name}"'
            )

    def write_transactional_info(self, rows, execution: Execution):
        # Get Data Source
        data_source = self._get_data_source(
            execution.source.source_name, execution.source.source_metadata[0]
        )
        # Get Data Frame
        df = data_source.get_data_frame(
            execution.source.source_name,
        ...
        if self._transactional_type == TransactionalType.UUID:
            new_df = pd.DataFrame(
                [{"uuid": row["uuid"], "timestamp": now} for row in rows]
            )
        elif self._transactional_type == TransactionalType.GCLID_TIME:
            new_df = pd.DataFrame(
                {"gclid": row["gclid"], "time": row["time"], "timestamp": now}
                for row in rows
            )
        df = df.append(new_df, ignore_index=True)
        # Upload file
        # Add _uploaded into path
        path = FileDataSource._append_filename_uploaded(
            execution.source.source_metadata[1]
        )
        bytes = data_source._get_file_from_data_frame(df).getbuffer().tobytes()
        FileProvider(
            path, self._dataflow_options, self._source_type, self._source_name, False
        ).write(bytes)

    def get_data_frame(
        self, source_name: str, path: str, is_uploaded: bool = False
    ) -> pd.DataFrame:
            ...
            if is_uploaded:
                if self._transactional_type == TransactionalType.UUID:
                    ...
                    raise NotImplementedError(
                        f'Transactional type not defined: {self._transactional_type.name}. Source="{self._source_name}". Destination="{self._destination_name}"'
                    )
            else:
                raise ValueError(
                    f'Unable to find file: "{path}". Source="{self._source_name}". Destination="{self._destination_name}"'
                )
        else:
    ...
    def _get_data_source(self, source_name: str, file_type: str):
        ...
        if file_type == "PARQUET":
            return ParquetDataSource(
                self._executions, self._transactional_type, self._dataflow_options
            )
        elif file_type == "CSV":
        ...
        raise ValueError(
            f'Data source not found. Please check your source in config (value={file_type}). Source="{self._source_name}". Destination="{self._destination_name}"'
        )
    ...
    def _update_dtypes(
        self, destination_type: DestinationType, col_names: list
    ) -> dict:
    ...
    def _get_data_frame_from_file(
        self, file: io.BytesIO, is_uploaded: bool
    ) -> pd.DataFrame:
        raise NotImplementedError(
            f'Data source not defined. Please call FileDataSource._get_data_source for defining the correct data source. Source="{self._source_name}". Destination="{self._destination_name}"'
        )
...
class ParquetDataSource(FileDataSource):
    def _get_data_frame_from_file(
        ...
        if is_uploaded:
            destination_type = (
                DestinationType.UPLOADED_GCLID_TIME
                if self._transactional_type == TransactionalType.GCLID_TIME
                else DestinationType.UPLOADED_UUID
            )
        ...
        if DataSchemas.validate_data_columns(cols, destination_type):
            ...
            raise ValueError(
                f'Data source incomplete. {DataSchemas.get_error_message(cols, self._destination_type)} Source="{self._source_name}". Destination="{self._destination_name}"'
            )
