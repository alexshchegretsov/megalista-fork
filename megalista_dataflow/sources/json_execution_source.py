...
class JsonExecutionSource(BaseBoundedSource):
    ...
    def read(self, range_tracker):
        ...
        logging.getLogger("megalista.JsonExecutionSource").info(
            f"Loading configuration JSON {json_url}..."
        )
        ...
        google_analytics_account_id = self._json_config.get_value(
            json_data, "GoogleAnalyticsAccountId"
        )
        campaign_manager_profile_id = self._json_config.get_value(
            json_data, "CampaignManagerProfileId"
        )

        if campaign_manager_profile_id is None:
            campaign_manager_profile_id = self._json_config.get_value(
                json_data, "CampaignManagerAccountId"
            )
        ...
        if schedules:
            ...
            logging.getLogger("megalista.JsonExecutionSource").warn(
                "No schedules found!"
            )
    ...
    def _read_sources(json_config, json_data):
        ...
        if sources_list:
            for row in sources_list:
                ...
                source = Source(
                    row["Name"], SourceType[row["Type"]], [row["Dataset"], row["Table"]]
                )
        ...
        if destinations_list:
            for row in destinations_list:
                ...
                destination = Destination(
                    row["Name"], DestinationType[row["Type"]], row["Metadata"]
                )
