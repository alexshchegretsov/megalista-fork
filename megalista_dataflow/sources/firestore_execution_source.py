...
class FirestoreExecutionSource(BaseBoundedSource):
    ...
    def __init__(self, setup_firestore_collection: ValueProvider):
        super().__init__()
        self._setup_firestore_collection = setup_firestore_collection

    def _do_count(self):
        collections = (
            firestore.Client().collection(self._setup_firestore_collection.get()).get()
        )
    ...
    def read(self, range_tracker):
        ...
        db = firestore.Client()
        entries = (
            db.collection(self._setup_firestore_collection.get())
            .where("active", "==", "yes")
            .stream()
        )
        ...
        campaign_manager_profile_id = account_data.get(
            "campaign_manager_profile_id", "empty"
        )
        ...
        if entries:
            for entry in entries:
                metadata = [
                    entry["bq_dataset"],
                    entry["bq_table"],
                ]  # TODO: flexibilize for other source types
                source = Source(
                    entry["source_name"], SourceType[entry["source"]], metadata
                )
        ...
        def create_metadata_list(entry):
            metadata_list = {
                ...
                "GA_MEASUREMENT_PROTOCOL": [
                    ...
                    "google_analytics_non_interaction",
                ],
                "GA_DATA_IMPORT": [
                    "google_analytics_property_id",
                    "google_analytics_data_import_name",
                ],
                "GA_USER_LIST_UPLOAD": [
                    "google_analytics_property_id",
                    "google_analytics_view_id",
                    "google_analytics_data_import_name",
                    "google_analytics_user_id_list_name",
                    "google_analytics_user_id_custom_dim",
                    "google_analytics_buyer_custom_dim",
                ],
                "CM_OFFLINE_CONVERSION": [
                    "campaign_manager_floodlight_activity_id",
            ...
            for m in metadata:
                ...
                if m == "metadata_padding":
                    ...
                    raise Exception(
                        f"Missing field in Firestore document for {entry_type}: {m}"
                    )
