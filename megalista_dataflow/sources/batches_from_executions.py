...
class BatchesFromExecutions(beam.PTransform):
    ...
    class _ReadDataSource(MegalistaUploader):
        ...
        ) -> Iterable[DataRowsGroupedBySource]:
        ...
        return (
            ...
            | beam.Filter(
                lambda executionGroupedBySource: functools.reduce(
                    lambda a, b: a or b,
                    [
                        execution.destination.destination_type == self._destination_type
                        for execution in executionGroupedBySource.executions
                    ],
                    False,
                )
            )
            | beam.ParDo(
                self._ReadDataSource(
            ...
            | beam.Map(
                lambda el: [
                    (execution, el.rows) for execution in iter(el.executions.executions)
                ]
            )
