...
def test_read_destinations(mocker, json_config):
    ...
    destinations: Dict[str, Destination] = JsonExecutionSource._read_destination(
    ...
    assert (
        destinations[key].destination_type
        == DestinationType.ADS_CUSTOMER_MATCH_CONTACT_INFO_UPLOAD
    )
    assert destinations[key].destination_metadata == [
        "Megalista - Testing - Contact Info",
        "ADD",
        "10",
    ]
...
def test_read_sources(mocker, json_config):
    ...
    sources: Dict[str, Source] = JsonExecutionSource._read_sources(
        json_config, json_data
    )
    ...
    assert sources[key].source_metadata == [
        "megalista_data",
...
def test_count(mocker, json_config):
    ...
    execution_source: JsonExecutionSource = JsonExecutionSource(
        json_config, StaticValueProvider(str, "")
    )
