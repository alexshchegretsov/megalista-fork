...
class GmailNotifier(ErrorNotifier):
    ...
    def notify(self, destination_type: DestinationType, errors: Iterable[Error]):
        ...
        if not self._should_notify():
            logger.info(
                f'Skipping sending emails notifying of errors: {", ".join(map(str, errors))}'
            )
            return
        ...
        message[
            "subject"
        ] = f"[Action Required] Megalista error detected - {destination_type.name}"
    ...
    def email_destinations(self) -> Iterable[str]:
        ...
        self._parsed_emails = list(
            map(
                lambda email: email.strip(),
                str(self._email_destinations.get()).split(","),
            )
        )
        return self._parsed_emails

    def _build_email_body(
        self, destination_type: DestinationType, errors: Iterable[Error]
    ):
...
class ErrorHandler:
    ...
    def __init__(
        self, destination_type: DestinationType, error_notifier: ErrorNotifier
    ):
